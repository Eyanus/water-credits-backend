/**
 * Allowlist Revoke Script
 *
 * Removes an asset (code + issuer) from the deployed Water Credits Soroban
 ...
 *
 * Usage:
 *   npx ts-node -r tsconfig-paths/register src/scripts/allowlist-revoke.ts \
 *     --asset-code USDC \
...
async function main(): Promise<void> {
  ...
  section('4/5  Simulating revoke_asset (dry-run)');
  ...
  if (!dryRun.success) {
    ...
    console.error('\nPossible causes:');
    console.error('  • The asset is not currently on the allowlist');
    console.error('  • The operator key does not have admin privileges on this contract');
    console.error('  • The contract does not expose a revoke_asset(code, issuer) method');
  ...
  if (networkConfig.name === 'mainnet') {
    warn(
      `${COLORS.bold}You are about to revoke an asset from the MAINNET allowlist.${COLORS.reset}`,
    );
    warn(
  ...
  try {
    ...
    while (attempts < maxAttempts) {
      if (statusResponse.status === SorobanRpc.Api.GetTransactionStatus.SUCCESS) {
        console.log();
        ok(
          `${COLORS.bold}${COLORS.green}Asset successfully revoked from the allowlist.${COLORS.reset}`,
        );
